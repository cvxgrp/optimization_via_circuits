...
def decentralized_admm_graph6(mu, L_smooth, R, Inductance, params=None):
    ...
    i_L_13_2 = i_L_13_1 + ( h / Inductance) * (e_13_2 - x3_2)
    i_L_23_2 = i_L_23_1 + ( h / Inductance) * (e_23_2 - x3_2)
    i_L_24_2 = i_L_24_1 + ( h / Inductance) * (e_24_2 - x4_2)
    i_L_34_2 = i_L_34_1 + ( h / Inductance) * (e_34_2 - x4_2)
    i_L_45_2 = i_L_45_1 + ( h / Inductance) * (e_45_2 - x5_2)
    i_L_46_2 = i_L_46_1 + ( h / Inductance) * (e_46_2 - x6_2)
    ...
    Delta_2 = d * (2/R) * ((e_12_2 - x1_2)**2 + (e_13_2 - x1_2)**2 \
                         + (e_23_2 - x2_2)**2 + (e_24_2 - x4_2)**2 \
...
def pg_extra_line2(mu, L_smooth, R, R12, params=None):
    # line graph with 3 nodes
    if params is not None:
        # verification mode: PEP
        problem = PEPit.PEP()
    ...
    i_L_12_1 = y_f1_1 + y_h1_1 - 1/R12 * (x2_1 - x1_1)
    # i_L_12_1 = problem.set_initial_point()
    w1_1 = R * i_L_12_1
    w2_1 = R * i_L_12_1

    # update
    e1_1 = R/R12 * x2_1 + ( 1 - R/R12 ) * x1_1 - R * y_h1_1 - w1_1
    e2_1 = R/R12 * x1_1 + ( 1 - R/R12 ) * x2_1 - R * y_h2_1 - w2_1
    x1_2, _, _ = proximal_step(e1_1, f1, R)
    x2_2, _, _ = proximal_step(e2_1, f2, R)
    i_L_12_2 = i_L_12_1 + 1/2 * 1/R12 * (x1_1 - x2_1)
    # w1_2 = R * i_L_12_2
    ...
    E_1 = gamma * (x1_1 - x_star)**2 + gamma * (x2_1 - x_star)**2 \
            + (L12/2) * (i_L_12_1 - y_f1_star - y_h1_star) ** 2 
    E_2 = gamma * (x1_2 - x_star)**2 + gamma * (x2_2 - x_star)**2 \
            + (L12/2) * (i_L_12_2 - y_f1_star - y_h1_star) ** 2   
    
    Delta_2 = d * 1/R12 * ((x2_1 - x1_1)**2) \
              + b * ( f1_1 - f1_star \
                    + f2_1 - f2_star \
                    + h1_1 - h1_star \
                    + h2_1 - h2_star)

...
def dadmm_C_graph6_2(mus, L_smooths, R, Capacitance, Inductance, params=None):
    ...
    e_46_2 = (x4_2 + x6_2) / 2
    i_L_12_2 = i_L_12_1 + ( h / Inductance[0]) * (e_12_2 - x1_2)      
    i_L_13_2 = i_L_13_1 + ( h / Inductance[0]) * (e_13_2 - x1_2)
    i_L_23_2 = i_L_23_1 + ( h / Inductance[0]) * (e_23_2 - x2_2)
    i_L_24_2 = i_L_24_1 + ( h / Inductance[0]) * (e_24_2 - x2_2)
    i_L_34_2 = i_L_34_1 + ( h / Inductance[0]) * (e_34_2 - x3_2)
    i_L_45_2 = i_L_45_1 + ( h / Inductance[1]) * (e_45_2 - x4_2)
    i_L_54_2 = i_L_54_1 + ( h / Inductance[1]) * (e_45_2 - x5_2)
    i_L_46_2 = i_L_46_1 + ( h / Inductance[0]) * (e_46_2 - x4_2)
    ...
    E_1 = gamma * ((e_12_1 - x_star)**2 + (e_13_1 - x_star)**2 + (e_23_1 - x_star)**2 + (e_24_1 - x_star)**2\
            ...
            + Inductance[0] * (i_L_12_1 - y1_12_star) ** 2 + Inductance[0] * (i_L_13_1 - y1_13_star) ** 2 \
            + Inductance[0] * (i_L_23_1 - y2_23_star) ** 2 + Inductance[0] * (i_L_24_1 - y2_24_star) ** 2 \
            + Inductance[0] * (i_L_34_1 - y3_34_star) ** 2 \
            + (Inductance[1]/2) * (i_L_45_1 + y5_star) ** 2 + (Inductance[1]/2) * (i_L_54_1 - y5_star) ** 2 \
            + Inductance[0] * (i_L_46_1 + y6_star) ** 2 \
    ...
    E_2 = gamma * ((e_12_2 - x_star)**2 + (e_13_2 - x_star)**2 + (e_23_2 - x_star)**2 + (e_24_2 - x_star)**2\
            ...
            + Inductance[0] * (i_L_12_2 - y1_12_star) ** 2 + Inductance[0] * (i_L_13_2 - y1_13_star) ** 2 \
            + Inductance[0] * (i_L_23_2 - y2_23_star) ** 2 + Inductance[0] * (i_L_24_2 - y2_24_star) ** 2 \
            + Inductance[0] * (i_L_34_2 - y3_34_star) ** 2 \
            + (Inductance[1]/2) * (i_L_45_2 + y5_star) ** 2 + (Inductance[1]/2) * (i_L_54_2 - y5_star) ** 2 \
            + Inductance[0] * (i_L_46_2 + y6_star) ** 2 \
...
def dadmm_C6_graph6(mu, L_smooth, R, Capacitance, Inductance, params=None):
    # graph with 6 nodes and 7 edges (1,2), (1,3), (2,3), (2,4), (3,4), (4,5), (4,6)
    # 1 -- 2 -- 4 -- 5
    #   \  |  /  \
    #      3       6 
    # D-ADMM with Capacitor at each e_jl
    if params is not None:
        # verification mode: PEP
        problem = PEPit.PEP()
        package = pep_func 
    ...
    x1_2, y1_2, f1_2 = proximal_step((1/2)*((R * i_L_12_1 + e_12_1) + (R * i_L_13_1 + e_13_1)), f1, R/2)
    x2_2, y2_2, f2_2 = proximal_step((1/3)*((R * i_L_21_1 + e_12_1) + (R * i_L_23_1 + e_23_1) \
                                            + (R * i_L_24_1 + e_24_1)), f2, R/3)
    x3_2, y3_2, f3_2 = proximal_step((1/3)*((R * i_L_31_1 + e_13_1) + (R * i_L_32_1 + e_23_1) \
                                            + (R * i_L_34_1 + e_34_1)), f3, R/3)
    x4_2, y4_2, f4_2 = proximal_step((1/4)*((R * i_L_42_1 + e_24_1) + (R * i_L_43_1 + e_34_1) \
                                            + (R * i_L_45_1 + e_45_1) + (R * i_L_46_1 + e_46_1)), f4, R/4)
    x5_2, y5_2, f5_2 = proximal_step(R * i_L_54_1 + e_45_1, f5, R)
    x6_2, y6_2, f6_2 = proximal_step(R * i_L_64_1 + e_46_1, f6, R)

    e_12_2 = e_12_1 - ( h / Capacitance) * (i_L_12_1 + i_L_21_1 + (2 * e_12_1 - x1_2 - x2_2) / R)
    e_13_2 = e_13_1 - ( h / Capacitance) * (i_L_13_1 + i_L_31_1 + (2 * e_13_1 - x1_2 - x3_2) / R)
    e_23_2 = e_23_1 - ( h / Capacitance) * (i_L_23_1 + i_L_32_1 + (2 * e_23_1 - x2_2 - x3_2) / R)
    e_24_2 = e_24_1 - ( h / Capacitance) * (i_L_24_1 + i_L_42_1 + (2 * e_24_1 - x2_2 - x4_2) / R)
    ...
    E_1 = (Capacitance / 2)  * ((e_12_1 - x_star)**2 + (e_13_1 - x_star)**2 + (e_23_1 - x_star)**2 \
                + (e_24_1 - x_star)**2 + (e_34_1 - x_star)**2 +(e_45_1 - x_star) ** 2 \
                + (e_46_1 - x_star)**2) +  \
            + (Inductance/2) * ((i_L_12_1 - y1_12_star) ** 2 + (i_L_21_1 + y1_12_star) ** 2 \
                            + (i_L_13_1 - y1_13_star) ** 2 + (i_L_31_1 + y1_13_star) ** 2 \
                            + (i_L_23_1 - y2_23_star) ** 2 + (i_L_32_1 + y2_23_star) ** 2\
                            + (i_L_24_1 - y2_24_star) ** 2 + (i_L_42_1 + y2_24_star) ** 2 \
                            + (i_L_34_1 - y3_34_star) ** 2 + (i_L_43_1 + y3_34_star) ** 2 \
                            + (i_L_45_1 + y5_star) ** 2 + (i_L_54_1 - y5_star) ** 2 \
                            + (i_L_46_1 + y6_star) ** 2 + (i_L_64_1 - y6_star) ** 2)

    E_2 = (Capacitance / 2) * ((e_12_2 - x_star)**2 + (e_13_2 - x_star)**2 + (e_23_2 - x_star)**2 \
                + (e_24_2 - x_star)**2 + (e_34_2 - x_star)**2 + (e_45_2 - x_star) ** 2 \
                + (e_46_2 - x_star)**2) +  \
            + (Inductance/2) *((i_L_12_2 - y1_12_star) ** 2 + (i_L_21_2 + y1_12_star) ** 2 \
                            + (i_L_13_2 - y1_13_star) ** 2 + (i_L_31_2 + y1_13_star) ** 2 \
                            + (i_L_23_2 - y2_23_star) ** 2 + (i_L_32_2 + y2_23_star) ** 2 \
                            + (i_L_24_2 - y2_24_star) ** 2 + (i_L_42_2 + y2_24_star) ** 2 \
                            + (i_L_34_2 - y3_34_star) ** 2 + (i_L_43_2 + y3_34_star) ** 2 \
                            + (i_L_45_2 + y5_star) ** 2 + (i_L_54_2 - y5_star) ** 2 \
